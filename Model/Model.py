...
from torch_geometric.nn import avg_pool, global_mean_pool
...
class KGNNModel(torch.nn.Module):
    def __init__(
        ...
        self.pool_to_channels = nn.Linear(64, 128)
    ...
    def forward(self, data):
        ...
        x = self.mggc3(x, edge_index, edge_attr)
        ...
        # 替换为全局池化
        x_2 = global_mean_pool(x, batch)  # 使用同样的 batch 索引做池化

        x = torch.cat([x_1, x_2], dim=1)  # 拼接 Set2Set + GlobalMeanPool
        x_2 = self.pool_to_channels(x_2)
