...
class SimpleGatedGraphConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr=None, edge_weight=None):
        ...
        if edge_attr is not None:
            weight = self.nn(edge_attr).view(-1, self.out_channels, self.out_channels)  # [E, C, C]
        ...
        for i in range(self.num_layers):
            ...
            if edge_attr is not None:
                ...
                m_new = self.propagate(edge_index, x=m, weight=None) #如果没有边特征，就只进行节点特征汇聚
    ...
    def message(self, x_j, weight=None, edge_weight=None): # 暂时没用上
        msg = x_j
    ...
    def update(self, aggr_out): # 暂时没用上
        if self.use_bias_for_update:
